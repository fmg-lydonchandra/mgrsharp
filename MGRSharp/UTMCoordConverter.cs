...
internal class UTMCoordConverter
{
    ...
    private long SetUTMParameters(double a, double f, long overrd)
    {
        ...
        if (a <= 0.0) /* Semi-major axis must be greater than zero */ Error_Code |= UTM_A_ERROR;
        if (inv_f < 250 || inv_f > 350) /* Inverse flattening must be between 250 and 350 */
            Error_Code |= UTM_INV_F_ERROR;
        if (overrd < 0 || overrd > 60) Error_Code |= UTM_ZONE_OVERRIDE_ERROR;
    ...
    public long ConvertGeodeticToUTM(double Latitude, double Longitude)
    {
        ...
        if (Latitude < MIN_LAT || Latitude > MAX_LAT) /* Latitude out of range */ Error_Code |= UTM_LAT_ERROR;
        if (Longitude < -PI || Longitude > 2 * PI) /* Longitude out of range */ Error_Code |= UTM_LON_ERROR;
        if (Error_Code == UTM_NO_ERROR)
        {
            /* no errors */
            if (Longitude < 0)
            ...
            if (Error_Code == UTM_NO_ERROR)
            {
                ...
                try
                {
                    var TM = TMCoord.FromLatLon(Angle.FromRadians(Latitude), Angle.FromRadians(Longitude),
                        UTM_a, UTM_f, Angle.FromRadians(Origin_Latitude),
    ...
    /**
         ...
         */
    public string Hemisphere => _Hemisphere;
...
//            Error_Code |= UTM_EASTING_ERROR;                       //that extend into another zone
        ...
        if (Error_Code == UTM_NO_ERROR)
        {
            ...
            try
            {
                var TM = TMCoord.FromTM(Easting, Northing,
    ...
    public double CentralMeridian => _Central_Meridian;
    ...
    public static LatLon ConvertWGS84ToNAD27(Angle latWGS, Angle lonWGS)
    {
        ...
        var deltaY = 130.0;
        var deltaZ = 190.0;
        var difA = WGS84_A - CLARKE_A;
        var difF = WGS84_F - CLARKE_F;

        var lat = latWGS.radians;
        var lon = lonWGS.radians;

        var f = 1 - CLARKE_B / CLARKE_A;
        var e2 = 2 * f - Math.Pow(f, 2);
        var Rn = CLARKE_A / Math.Sqrt(1 - e2 * Math.Pow(Math.Sin(lat), 2.0));
        var Rm = CLARKE_A * (1 - e2) / Math.Pow(1 - e2 * Math.Pow(Math.Sin(lat), 2.0), 1.5);
        var errLon = (-1 * deltaX * Math.Sin(lon) + deltaY * Math.Cos(lon)) / (Rn * Math.Cos(lat));
        var errLat = (-1 * deltaX * Math.Sin(lat) * Math.Cos(lon) - deltaY * Math.Sin(lat) * Math.Sin(lon)
                      + deltaZ * Math.Cos(lat)
                      + difA * (Rn * e2 * Math.Sin(lat) * Math.Cos(lat)) / CLARKE_A
                      + difF * (Rm * CLARKE_A / CLARKE_B + Rn * CLARKE_B / CLARKE_A) * Math.Sin(lat) * Math.Cos(lat)) /
                     Rm;
