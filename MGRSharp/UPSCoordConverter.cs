...
public class UPSCoordConverter
{
    ...
    private PolarCoordConverter polarConverter = new();
    ...
    public long SetUPSParameters(double a, double f)
    {
        ...
        if (a <= 0.0) /* Semi-major axis must be greater than zero */ return UPS_A_ERROR;
        if (inv_f < 250 || inv_f > 350) /* Inverse flattening must be between 250 and 350 */ return UPS_INV_F_ERROR;
    ...
    {
        if (latitude < -MAX_LAT || latitude > MAX_LAT) /* latitude out of range */ return UPS_LAT_ERROR;
        if (latitude < 0 && latitude > MIN_SOUTH_LAT)
            return UPS_LAT_ERROR;
        if (latitude >= 0 && latitude < MIN_NORTH_LAT)
            return UPS_LAT_ERROR;

        if (longitude < -PI || longitude > 2 * PI) /* slam out of range */ return UPS_LON_ERROR;
