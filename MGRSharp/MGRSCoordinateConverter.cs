...
internal class MGRSCoordConverter
{
    ...
    public double MGRS_f => _MGRS_f;

    /** @return Semi-major axis of ellipsoid in meters */
    public double MGRS_a => _MGRS_a;

    /** @return Latitude band letter */
    private long LastLetter => lastLetter;

    /** @return 2-letter code for ellipsoid */
    public string MGRS_Ellipsoid_Code => _MGRS_Ellipsoid_Code;
    ...
    public long ConvertMGRSToGeodetic(string MGRSString)
    {
        ...
        {
            var UTM = ConvertMGRSToUTM(MGRSString);
            if (UTM != null)
            {
        ...
        {
            var UPS = ConvertMGRSToUPS(MGRSString);
            if (UPS != null)
            {
    ...
    private MGRSComponents BreakMGRSString(string MGRSString)
    {
        ...
        while (i < MGRSString.Length && MGRSString[i] == ' ') i++; /* skip any leading blanks */
        j = i;
        while (i < MGRSString.Length && char.IsDigit(MGRSString[i])) i++;
        num_digits = i - j;
        if (num_digits <= 2)
        {
            if (num_digits > 0)
        ...
        while (i < MGRSString.Length && char.IsLetter(MGRSString[i])) i++;
        num_letters = i - j;
        if (num_letters == 3)
        {
            /* get letters */
    ...
    private long CheckZone(string MGRSString)
    {
        ...
        while (i < MGRSString.Length && MGRSString[i] == ' ') i++;
        j = i;
        while (i < MGRSString.Length && char.IsDigit(MGRSString[i])) i++;
        num_digits = i - j;
        if (num_digits > 2)
    ...
    private UTMCoord ConvertMGRSToUTM(string MGRSString)
    {
        ...
        var MGRS = BreakMGRSString(MGRSString);
        if (MGRS == null)
        {
            error_code |= MGRS_STRING_ERROR;
        }
    ...
    public long convertGeodeticToMGRS(double latitude, double longitude, int precision)
    {
        ...
        long error_code = MGRS_NO_ERROR;
        if (latitude < -PI_OVER_2 || latitude > PI_OVER_2) /* Latitude out of range */ error_code = MGRS_LAT_ERROR;

        if (longitude < -PI || longitude > 2 * PI) /* Longitude out of range */ error_code = MGRS_LON_ERROR;
    ...
    public string MGRSString => _MGRSString;
    ...
    private long ConvertUPSToMGRS(string Hemisphere, double Easting, double Northing, long Precision)
    {
        ...
        var letters = new long[MGRS_LETTERS]; /* Number location of 3 letters in alphabet     */
        double divisor;
        int index;
        long error_code = MGRS_NO_ERROR;
...
//                false_northing = UPS_Constant_Table.get((int) letters[0]).false_northing;
    ...
    private long ConvertUTMToMGRS(long Zone, double Latitude, double Easting, double Northing, long Precision)
    {
        ...
        if (error_code == MGRS_NO_ERROR)
        {
            ...
            while (grid_northing >= TWOMIL) grid_northing = grid_northing - TWOMIL;
    ...
    private long GetLatitudeLetter(double latitude)
    {
        double temp;
        long error_code = MGRS_NO_ERROR;
        var lat_deg = latitude * RAD_TO_DEG;
    ...
    private long MakeMGRSString(long Zone, long[] Letters, double Easting, double Northing, long Precision)
    {
        ...
        {
            int i;
            var length = sEast.Length;
            for (i = 0; i < Precision - length; i++) sEast = "0" + sEast;
        }
        ...
        {
            int i;
            var length = sNorth.Length;
            for (i = 0; i < Precision - length; i++) sNorth = "0" + sNorth;
        }
